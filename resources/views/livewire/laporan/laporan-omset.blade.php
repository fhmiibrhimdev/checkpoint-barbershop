                        ...
                        <table class='tw-w-full tw-table-auto'>
                            <thead class="tw-sticky tw-top-0">
                                <tr class="tw-text-gray-700">
                                    ...
                                    <th class="tw-whitespace-nowrap">Total Akhir</th>
                                    <th class="tw-whitespace-nowrap">Uang DP</th>
                                    <th class="tw-whitespace-nowrap">Sudah Dibayar</th>
                                    <th class="tw-whitespace-nowrap">Sisa Bayar</th>
                                </tr>
                            </thead>
                            <tbody>
                                @php
                                use Carbon\Carbon;
                                // Nomor urut yang konsisten antar halaman
                                $no = ($data->currentPage() - 1) * $data->perPage() + 1;

                                // Kelompokkan per TANGGAL (tanpa jam) di level koleksi paginator
                                $groups = $data->getCollection()->groupBy(function ($item) {
                                return Carbon::parse($item->tanggal)->toDateString(); // 'YYYY-MM-DD'
                                });
                                @endphp
                                ...
                                <tr class="">
                                    <td colspan="8" class="tw-font-semibold tw-text-gray-700">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($row->status == '2')
                                        <span class="badge tw-bg-red-100 tw-text-red-600">Belum Lunas</span>
                                        @elseif ($row->status == '3')
                                        <span class="badge tw-bg-green-100 tw-text-green-600">Lunas</span>
                                        @endif
                                    </td>
                                    <td>@money($row->total_akhir)</td>
                                    <td>
                                        @if ($row->status == '3')
                                        @money(0)
                                        @else
                                    ...
                                    <td>
                                        @if ($row->status == '3')
                                        @money($row->total_akhir)
                                        @else
                                        @money($row->sudah_dibayar)
                                        @endif
                                    </td>
                                    <td>
                                        @if ($row->status == '3')
