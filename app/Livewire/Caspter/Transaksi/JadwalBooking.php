...
class JadwalBooking extends Component
{
    ...
    public function render()
    {
        ...
        $data = DB::table('transaksi')->select('transaksi.id', 'transaksi.no_transaksi', 'transaksi.tanggal', 'daftar_pelanggan.nama_pelanggan', 'daftar_pelanggan.no_telp',  'transaksi.total_akhir', 'transaksi.status', 'detail.nama_item', 'detail.deskripsi_item', 'jumlah.jumlah_produk', 'kategori_pembayaran.nama_kategori', 'cabang_lokasi.nama_cabang', 'detail.id_karyawan')
    ...
    public function cartProduk($id)
    {
        ...
        $this->id_karyawan = Auth::user()->id - 1;

        if (in_array($this->check_id_kategori, ['2', '3'])) { // Kategori: Jasa Barbershop & Treatment
            $this->karyawans = DB::table('daftar_karyawan')
                ...
                ->distinct()
                ->get();
        } else if ($this->check_id_kategori == "1") {
            $this->karyawans = DB::table('daftar_karyawan')
                ->select('daftar_karyawan.id', 'users.name')
                ->join('users', 'users.id', '=', 'daftar_karyawan.id_user')
                ->where('daftar_karyawan.id', $this->id_karyawan)
