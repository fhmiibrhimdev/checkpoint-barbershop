...
class Transaksi extends Component
{
    ...
    // Table: transaksi
    public $id_cabang, $id_user, $no_transaksi, $tanggal, $id_pelanggan, $catatan, $total_pesanan, $total_komisi, $total_sub_total, $total_diskon, $total_akhir, $total_hpp, $laba_bersih, $id_metode_pembayaran, $jumlah_dibayarkan, $kembalian, $status;
    ...
    public function render()
    {
        ...
        $data = DB::table('transaksi')->select('transaksi.id', 'transaksi.no_transaksi', 'transaksi.tanggal', 'daftar_pelanggan.nama_pelanggan', 'daftar_pelanggan.no_telp',  'transaksi.total_akhir', 'transaksi.status', 'detail.nama_item', 'detail.deskripsi_item', 'jumlah.jumlah_produk', 'kategori_pembayaran.nama_kategori')
            ...
            ->where('transaksi.id_cabang', $this->filter_id_cabang) // Filter berdasarkan id_cabang
        ...
        $transaksiHariIni = DB::table('transaksi')
            ->selectRaw('
                ...
                SUM(CASE 
                        ...
                        THEN LEAST(
                            CAST(jumlah_dibayarkan AS DECIMAL(15,2)), 
                            CAST(total_akhir AS DECIMAL(15,2))
                        ) 
                        ELSE 0 
                    END) as total_tunai,
                SUM(CASE 
    ...
    public function store()
    {
        ...
        try {
            if ($this->cartItems == []) {
                throw new \Exception("Silahkan isi items dulu ke keranjang!");
            }
            ...
            if ($this->flag_reset_kunjungan) {
                ...
                DaftarPelanggan::where('id', $this->id_pelanggan)
    ...
    {
        $data = DB::table('produk')->select('produk.id', 'produk.id_kategori', 'nama_item', 'harga_jasa', 'harga_pokok', 'nama_kategori', 'produk.deskripsi')
            ->join('kategori_produk', 'kategori_produk.id', 'produk.id_kategori')
            ->where('produk.id', $id)
            ->first();
    ...
    public function getRingkasanTransaksi()
    {
        ...
        $this->laba_bersih     = $total_sub_total - $total_diskon - $total_komisi - $total_hpp;
    }
    ...
    public function totalPembayaranCashModal()
    {
        ...
        $this->detailPembayaranCash = $gabungan;

        // Jika ingin total jumlah semua pembayaran cash:
        $this->totalDetailPembayaranCash = collect($gabungan)->sum('jumlah');
    }
    ...
    public function totalPembayaranTransferModal()
    {
        ...
        $this->detailPembayaranTransfer = $gabungan;

        // Jika ingin total jumlah semua pembayaran cash:
        $this->totalDetailPembayaranTransfer = collect($gabungan)->sum('jumlah');
    }
    ...
    public function totalPiutangModal()
    {
        $this->detailPiutang = DB::table('transaksi')
            ...
            ->where('id_cabang', $this->filter_id_cabang)
            ->get()
            ->filter(fn($item) => (int) $item->jumlah < 0) // hanya ambil yang jumlah > 0
            ->values(); // reset index agar clean di Blade
